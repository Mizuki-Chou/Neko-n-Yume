...
public final class GrowthMath {
     ...
     *
     * <p>
     * 与旧 Cat.levelFromExperience 语义完全一致（含 10000 级安全上限）：
     * 返回满足 cumXp(L) <= totalExperience 的最大等级。
     * </p>
     *
     * <p>
    ...
    public static int levelFromExperience(
        ...
        /*
         * 上界二分：
         * low/high 始终满足 cumXp(low) <= E < cumXp(high+1)，
         * 收敛后 low 即最大满足等级。
         */
        int low = 1;
     ...
     *
     * <p>
     * 与旧 Cat.meowRankFromPower 语义完全一致（含 10000 阶安全上限）：
     * 返回满足 cumMeow(N) <= totalMeowPower 的最大喵阶。
     * </p>
     *
     * <p>
